...
SynthDef("tapeslow", {
    ...
    imp=Impulse.kr(rate);
    tries = duration/(16*60/bpm);
    prob=times/tries;
    length_time=Demand.kr(imp,Dseq([1,1,2,2,2,2,4,4,4]*60/bpm,inf));
    env=EnvGen.kr(Env.new([0,0.9,0.9,0],[0.2,length_time-0.2-0.2,0.2]),imp*(LFNoise0.kr(imp).range(0,1)<prob));
...
SynthDef("toggle", {
    ...
    var tries = BufDur.ir(0)/(16*60/bpm);
    var prob=times/tries;
    var length_time=Demand.kr(imp,Dseq([1,1,2,2,2,2,4,4,4,8,8,16,32]*60/bpm,inf));
    var env=EnvGen.ar(Env.new([0,1,1,0],[0.2,length_time-0.2-0.2,0.2]),imp*(LFNoise0.kr(imp).range(0,1)<prob));
    snd = snd * (1-env);
    Out.ar(out, snd);
}).load(nrtServer);
